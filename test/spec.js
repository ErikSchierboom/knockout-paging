...
describe("paged extender", function() {
  ...
  beforeEach(function() {
    ...
    singlePageObservableArray = ko
      .observableArray([1])
      .extend({ paged: options });
    smallNumberPagesObservableArray = ko
      .observableArray(createRange(1, 7))
      .extend({ paged: options });
    largeNumberPagesObservableArray = ko
      .observableArray(createRange(1, 30))
      .extend({ paged: options });
    smallNumberPagesComputedArray = ko
  ...
  context("on regular observable", function() {
    context("with non-array value", function() {
      it("throws", function() {
        ...
        expect(
          regularObservable.extend.bind(regularObservable, { paged: {} })
        ).to.throw(Error);
      });
    });
  ...
  context("on computed observable that does not return an array", function() {
    it("throws", function() {
      ...
      expect(
        computedObservable.extend.bind(computedObservable, { paged: {} })
      ).to.throw(Error);
    });
  });
  ...
  context("on computed observable that returns an array", function() {
    ...
    context("on multi-page paged computed array", function() {
      it("itemCount is the same as for multi-page paged observable array", function() {
        expect(smallNumberPagesComputedArray.itemCount()).to.equal(
          smallNumberPagesObservableArray.itemCount()
        );
      });
      it("pageItems is the same as for multi-page paged observable array", function() {
        expect(smallNumberPagesComputedArray.pageItems()).to.deep.equal(
          smallNumberPagesObservableArray.pageItems()
        );
      });
      it("pages is the same as for multi-page paged observable array", function() {
        expect(smallNumberPagesComputedArray.pages()).to.deep.equal(
          smallNumberPagesObservableArray.pages()
        );
      });
    });
  });
  ...
  context("on single-page paged observable array", function() {
    it("itemCount is number of elements in array", function() {
      expect(singlePageObservableArray.itemCount()).to.equal(
        singlePageObservableArray().length
      );
    });
    ...
    it("lastItemOnPage is is number of elements in array", function() {
      expect(singlePageObservableArray.lastItemOnPage()).to.equal(
        singlePageObservableArray().length
      );
    });
    ...
    it("pageItems returns all array elements", function() {
      expect(singlePageObservableArray.pageItems()).to.deep.equal(
        singlePageObservableArray()
      );
    });
  ...
  context("on multi-page paged observable array", function() {
    context("with current page is first page", function() {
      ...
      it("itemCount is number of elements in array", function() {
        expect(smallNumberPagesObservableArray.itemCount()).to.equal(
          smallNumberPagesObservableArray().length
        );
      });
      ...
      it("pageItems returns all array elements on page", function() {
        expect(smallNumberPagesObservableArray.pageItems()).to.deep.equal([
          1,
          2,
          3
        ]);
      ...
      it("toNextPage increments pageNumber", function() {
        ...
        expect(smallNumberPagesObservableArray.pageNumber()).to.equal(
          oldPageNumber + 1
        );
      });
      ...
      it("toLastPage sets pageNumber to last page", function() {
        ...
        expect(smallNumberPagesObservableArray.pageNumber()).to.equal(
          smallNumberPagesObservableArray.pageCount()
        );
      });
    });
  ...
  context("constructor", function() {
    context("without options", function() {
      var pagedArrayWithoutOptions, emptyOptions;

      beforeEach(function() {
        emptyOptions = {};
        pagedArrayWithoutOptions = ko
          .observableArray([])
          .extend({ paged: emptyOptions });
      });
      ...
      it("pageGenerator is default", function() {
        expect(pagedArrayWithoutOptions.pageGenerator).to.equal(
          ko.paging.generators["default"]
        );
      });
    ...
    context("with options", function() {
      var pagedArrayWithOptions, options;

      beforeEach(function() {
        options = { pageNumber: 3, pageSize: 25 };
        pagedArrayWithOptions = ko
          .observableArray(createRange(1, 7))
          .extend({ paged: options });
      });
      ...
      pageGeneratorNames.forEach(function(pageGeneratorName) {
        it("pageGenerator is equal to page generator with supplied option value", function() {
          expect(pagedArrayWithOptions.pageGenerator).to.equal(
            ko.paging.generators[pageGeneratorName]
          );
        });
      });

      it("pageGenerator is equal to custom page generator with supplied option value", function() {
        var customPageGenerator = function(pagedObservable) {
          return [];
        };
      ...
      numbersLessThenZero.forEach(function(invalidNumber) {
        it("pageNumber less than 1 throws", function() {
          ...
          expect(
            pagedArrayWithOptions.extend.bind(pagedArrayWithOptions, {
              paged: options
            })
          ).to.throw(Error);
        });
  ...
  context("page generators", function() {
    context("default page generator", function() {
      beforeEach(function() {
        singlePageObservableArray["pageGenerator"] =
          ko.paging.generators["default"];
        smallNumberPagesObservableArray["pageGenerator"] =
          ko.paging.generators["default"];
        largeNumberPagesObservableArray["pageGenerator"] =
          ko.paging.generators["default"];
      });

      it("works for single page", function() {
        expect(singlePageObservableArray.pages()).to.deep.equal(
          createRange(1, 1)
        );
      });

      it("works for small numbers of pages", function() {
        expect(smallNumberPagesObservableArray.pages()).to.deep.equal(
          createRange(1, 3)
        );
      });

      it("works for large number of pages", function() {
        expect(largeNumberPagesObservableArray.pages()).to.deep.equal(
          createRange(1, 10)
        );
      });
